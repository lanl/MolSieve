        ...
        switch (this.state.newStep) {
            case OVITO:
                axios.get('/api/get_ovito_modifiers').then((response) => {
                    const index = this.state.steps.length;
                    const options = response.data.map((val, idx) => (
                    ...
                    const select = (
                        <Select
                            defaultValue="AcklandJonesModifier"
                            onChange={(e) => {
                    ...
                    const newStep = { type: OVITO, value: 'AcklandJonesModifier', render: select };
