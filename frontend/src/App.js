...
class App extends React.Component {
	...
	axios.get('/pcca', {params: {
		...
		new_traj.fuzzy_memberships[idx] = clustered_data.fuzzy_memberships[idx];
            ...
            this.state.trajectories[state.name] = new_traj;
	    axios.get('/load_sequence', {params : {'run': state.name, 'properties': state.clicked.toString()}}).then(response => { 
		this.setState({isLoading: false});
		this.state.trajectories[state.name].sequence = response.data;	    	    
		this.state.trajectories[state.name].properties = state.clicked;            
	    });

	    console.log(this.state.trajectories[state.name]);
	}).catch(e => {
	    alert(e);
	});
    }    
    ...
    render() {
	    ...
	    return (<div className="App">
			...
			<LoadRunModal load_trajectory={this.load_trajectory} isOpen={this.state.currentModal === RUN_MODAL}
				      lastEvent={this.state.lastEvent} closeFunc={this.toggleModal(RUN_MODAL)}/>
			<Modal isOpen={this.state.isLoading}><ReactLoading type='spin' color='black' height='20%' width='20%' /></Modal>
		    </div>
		   );	
    }
}
