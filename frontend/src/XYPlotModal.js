...
class XYPlotModal extends React.Component {
	...
	if (this.props.isOpen) {	   	    
		...
		return (<option value={property}>{property}</option>);
            ...
            return (<Modal onRequestClose={this.closeFunc} isOpen={this.props.isOpen}>
		       <h1>{this.props.title}</h1>
		       <label htmlFor="select_x_attribute">X attribute: </label>
			<select name="select_x_attribute" id="select_x_attribute" defaultValue={this.state.x_attribute} onChange={(e) => { this.setX(e) }}>{options}</select>
